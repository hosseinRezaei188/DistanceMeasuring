...
namespace Distance_Measuring.DataProvider
{
    public class HistoryDataProvider : IHistoryDataProvider
    {
        ...
        {
             sqlDataContextcs.RequestHistory.
               AsEnumerable().Where(x => x.User.ID == UserID);

            List<RequestHistory> users =  sqlDataContextcs.RequestHistory.
               AsEnumerable().Where(x => x.User.ID == UserID).ToList();

            IEnumerable<UserVM> userVmList =
                mapper.Map<IEnumerable<User>, IEnumerable<UserVM>>(users);

...
namespace Distance_Measuring.DataProvider
{
    public class UserDataProvider : IDataProvider.IUserDataProvider
    {
        ...
        {
            User user;
            GetUserIfExist(userViewModel.UserName, out user);

            //check if user is exist
            if (user != null)
            {

                bool flag = Utils.VerifyPasswordHash(userViewModel.Password, user.PasswordHash, user.PasswordSalt);
        ...
        void GetUserIfExist(string userName, out User user)
        {
            //check if user is exist with this username
            user = sqlDataContextcs.User.
